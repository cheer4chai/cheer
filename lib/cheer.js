    ...
    _createClass(cookie, [{
        ...
        */
        ...
        value: function get(sName) {
            return _param2.default.getParam(window.document.cookie, sName, "; ", "=");
        }
    }]);
...
var string = function () {
    ...
    _createClass(string, [{
        ...
        value: function render(template, data) {
            var spliter = /\<\%.*?\%\>/g;
            var plain = template.split(spliter);
            var dynamic = template.match(spliter).map(function (str) {
                return str.startsWith('<%=') ? 'yield(' + str.slice(3, -2).trim() + ')' : str.slice(2, -2).trim();
            });
            var code = plain.map(function (txt, i) {
                return i in dynamic ? 'yield(' + JSON.stringify(txt) + ')\n' + dynamic[i] + '\n' : 'yield(' + JSON.stringify(txt) + ')\n';
            }).join('');
            var params = Object.getOwnPropertyNames(data);
            var output = [];
            new (Function.prototype.bind.apply(Function, [null].concat(_toConsumableArray(params), ["yield", code])))().apply(undefined, _toConsumableArray(params.map(function (name) {
                return data[name];
            })).concat([function (t) {
        ...
        key: 'dateFormat',
        value: function dateFormat(date, format) {
